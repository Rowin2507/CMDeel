<?php

// include '../assets/includes/config.php';s

$user_name = $_GET['username'];
$query = "SELECT * FROM cmdeel_users WHERE username='$user_name'";  
$result = mysqli_query($dbc, $query) or die ('Hmm, hier gaat iets niet helemaal goed...');
while($row = mysqli_fetch_array($result)) {
    $user_id = $row['user_id'];
    $name = $row['name'];
    $username = $row['username'];
    $profile_picture = $row['profile_picture'];
    $mail = $row['mail'];
    $bio = $row['description'];

    // CHECK IF YOU ARE THE USER 
    if ($username == $session_username) {
        $button = '<a class="button button-secondary" href="account">Profiel bewerken</a>';
    } else {
        $button = '<a class="button button-secondary" href="mailto:' . $mail . '">Kom in contact</a>';
    }


    echo '
    <section class="user-featured">
        <div class="user-featured-content">
            <div class="user-featured-content-item">
                <img src="assets/user-uploads/user-profiles/' . $profile_picture . '" alt="Profielfoto van ' . $user_name . '">
                <div class="user-featured-content-name">
                    <h2>' . $name . '</h2>
                    <h3>' . $username . '</h3>
                    ' . $button . '
                </div>
            </div>
            <div class="user-featured-content-item">
                <p>' . $bio . '</p>
            </div>
        </div>
    </section>
    <section class="user-uploads">
    <div class="overview-shots-content">
    ';
    

// GET UPLOADS (SHOTS) FROM DATABASE
$query = "SELECT * FROM cmdeel_uploads WHERE user_id='$user_id' ORDER BY upload_id DESC";
$result = mysqli_query($dbc, $query) or die ('Hmm, hier gaat iets niet helemaal goed...');

while($row = mysqli_fetch_array($result)) {
    $upload_id = $row['upload_id'];
    $upload_title = $row['upload_title'];
    $upload_file = $row['upload_file'];


        echo '
        <div class="overview-shots-content-item shot-' . $upload_id . '">
            <div class="shot-content">
                <a href="index.php?page=useruploads&upload_id=' . $upload_id . '" class="shot-file" id="' . $upload_id . '">
                    <picture>
                        <img src="assets/user-uploads/user-content/' . $upload_file . '" alt="' . $upload_title . '">
                    </picture>
                </a>
                <span class="shot-info">
                    <strong id="shot-info-' . $upload_id . '">' . $upload_title . '</strong>
                    <ul class="shot-actions">
                        <li class="shot-pin">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 50.12 49.97">
                                <path class="cls-1" d="M44,6.08C40.25,2.33,35.83,0,32.47,0a4.89,4.89,0,0,0-3.55,1.32c-1.48,1.48-1.71,3.93-.65,6.91a15.13,15.13,0,0,0,.59,1.41L22.17,16.1A12,12,0,0,0,18,15.35c-5.94,0-10.42,4.28-10.61,4.46a1.17,1.17,0,0,0-.27.42c-.08.25-1.94,6,4.92,14.41L1.92,44.6a.26.26,0,0,0-.08.13L0,49.51a.36.36,0,0,0,.08.37A.36.36,0,0,0,.34,50a.27.27,0,0,0,.13,0l5-2.06.11-.07,9.9-9.63a18.81,18.81,0,0,0,12,4.51,13.54,13.54,0,0,0,2.91-.28,1.09,1.09,0,0,0,.55-.34c3.14-3.64,5.56-8.13,2.45-14.55L40,21a12.22,12.22,0,0,0,5.33,1.5,4.9,4.9,0,0,0,3.56-1.32C51.72,18.27,49.6,11.65,44,6.08ZM30.76,12.87a.75.75,0,0,0-.65.21.78.78,0,0,0,0,1.11.79.79,0,0,0,1.11,0,.72.72,0,0,0,.2-.43c.36.47.75.93,1.15,1.39a.54.54,0,0,0-.24.16.77.77,0,0,0,0,1.1.78.78,0,0,0,1.1,0,.9.9,0,0,0,.14-.2c.06.05.11.11.17.17.34.35.69.67,1,1a.69.69,0,0,0-.24.15.79.79,0,0,0,0,1.11.78.78,0,0,0,1.1,0,.81.81,0,0,0,.2-.36c.57.47,1.15.9,1.74,1.3a.77.77,0,0,0-.82,1.28,1.11,1.11,0,0,0,.16.11l-3.37,3.37a.62.62,0,0,0-.11-.17.79.79,0,0,0-1.11,0,.8.8,0,0,0,0,1.11,1.13,1.13,0,0,0,.18.12l-3.37,3.38a.5.5,0,0,0-.13-.19.77.77,0,0,0-1.13,1.06,11.66,11.66,0,0,1-8-8.41l2.21-2.13a.76.76,0,0,0,.22.58.79.79,0,0,0,1.11,0,.78.78,0,0,0,0-1.11.76.76,0,0,0-.55-.23l3.66-3.53a.8.8,0,0,0,.2.45.79.79,0,0,0,1.11,0,.78.78,0,0,0,0-1.11.79.79,0,0,0-.43-.21l2.46-2.37C30.18,12,30.46,12.45,30.76,12.87ZM29.59,40.4a13.8,13.8,0,0,1-2.08.15,17,17,0,0,1-12.73-6c-6.44-7.16-5.9-12.27-5.67-13.39.89-.78,4.48-3.65,8.9-3.65a9.55,9.55,0,0,1,2.39.31L18,20.13a1,1,0,0,0-.3,1C19.9,30.62,28.05,31.91,28.4,32h.15a1.08,1.08,0,0,0,.76-.31l2.52-2.54C33.75,33.64,32.39,37.06,29.59,40.4ZM38,19.92a.67.67,0,0,0-.09-.14l.14.1Zm9.34-.27a2.81,2.81,0,0,1-2,.69c-2.76,0-6.7-2.15-10-5.46a21,21,0,0,1-5-7.37c-.75-2.12-.69-3.83.16-4.68a2.74,2.74,0,0,1,2-.69c2.77,0,6.7,2.14,10,5.46C47.48,12.56,49,17.93,47.32,19.65Z"/><path class="cls-1" d="M21.26,21.9A.79.79,0,0,0,22.37,23a.8.8,0,0,0,0-1.11A.79.79,0,0,0,21.26,21.9Z"/><path class="cls-1" d="M23.48,24.13a.78.78,0,0,0,0,1.11.78.78,0,1,0,1.1-1.11A.78.78,0,0,0,23.48,24.13Z"/><path class="cls-1" d="M25.7,26.35a.78.78,0,0,0,0,1.11.78.78,0,1,0,1.1-1.1A.78.78,0,0,0,25.7,26.35Z"/><path class="cls-1" d="M25.69,20.82a.78.78,0,1,0,0,1.1A.78.78,0,0,0,25.69,20.82Z"/><path class="cls-1" d="M26.8,23a.78.78,0,0,0,0,1.1.79.79,0,0,0,1.11,0,.8.8,0,0,0,0-1.11A.78.78,0,0,0,26.8,23Z"/><path class="cls-1" d="M29,25.26a.78.78,0,0,0,0,1.11.79.79,0,0,0,1.11,0,.8.8,0,0,0,0-1.11A.79.79,0,0,0,29,25.26Z"/><path class="cls-1" d="M26.79,18.6a.77.77,0,0,0,0-1.1.78.78,0,1,0-1.1,1.1A.77.77,0,0,0,26.79,18.6Z"/><path class="cls-1" d="M27.9,20.82a.79.79,0,0,0,1.11,0,.78.78,0,0,0,0-1.11.78.78,0,1,0-1.11,1.1Z"/><path class="cls-1" d="M30.12,23.05a.78.78,0,1,0,0-1.11A.79.79,0,0,0,30.12,23.05Z"/><path class="cls-1" d="M30.11,17.51a.78.78,0,0,0,0-1.11.8.8,0,0,0-1.11,0,.79.79,0,0,0,0,1.11A.78.78,0,0,0,30.11,17.51Z"/><path class="cls-1" d="M32.33,19.74a.78.78,0,1,0-1.11,0A.79.79,0,0,0,32.33,19.74Z"/><path class="cls-1" d="M34.55,22a.78.78,0,0,0,0-1.11.79.79,0,0,0-1.11,0,.8.8,0,0,0,0,1.11A.79.79,0,0,0,34.55,22Z"/>
                            </svg>
                        </li>
                        <li class="shot-heart active">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 455.52">
                                <path d="M256,455.52a30,30,0,0,1-19.79-7.44C215.52,430,195.58,413,178,398l-.09-.07C126.32,354,81.77,316,50.78,278.61,16.14,236.81,0,197.17,0,153.87,0,111.8,14.43,73,40.62,44.58,67.12,15.83,103.49,0,143,0c29.56,0,56.62,9.34,80.45,27.77a164.53,164.53,0,0,1,32.52,34,164.57,164.57,0,0,1,32.53-34C312.35,9.34,339.42,0,369,0c39.54,0,75.91,15.83,102.42,44.58C497.58,73,512,111.8,512,153.87c0,43.3-16.13,82.94-50.78,124.74-31,37.4-75.53,75.35-127.1,119.31-17.63,15-37.6,32-58.33,50.17A30,30,0,0,1,256,455.52ZM143,30C112,30,83.43,42.39,62.66,64.91,41.59,87.76,30,119.36,30,153.87c0,36.42,13.53,69,43.88,105.61,29.33,35.39,73,72.57,123.48,115.62l.09.08c17.66,15.05,37.68,32.11,58.52,50.33,21-18.25,41-35.34,58.7-50.42,50.52-43,94.14-80.22,123.47-115.61C468.48,222.86,482,190.29,482,153.87c0-34.51-11.6-66.11-32.67-89C428.58,42.39,400,30,369,30c-22.76,0-43.65,7.24-62.1,21.5C290.43,64.21,279,80.29,272.26,91.54a18.93,18.93,0,0,1-32.52,0C233,80.29,221.57,64.21,205.13,51.49,186.68,37.23,165.79,30,143,30Z"/><path d="M161.08,131.11A9.45,9.45,0,1,0,153,120.4,9.46,9.46,0,0,0,161.08,131.11Z"/><path d="M123.34,125.73A9.48,9.48,0,1,0,115.26,115,9.5,9.5,0,0,0,123.34,125.73Z"/><path d="M85.6,120.3a9.47,9.47,0,1,0-8.08-10.71A9.45,9.45,0,0,0,85.6,120.3Z"/><path d="M188,211.51A9.47,9.47,0,1,0,180,200.78,9.53,9.53,0,0,0,188,211.51Z"/><path d="M142.24,195.4a9.53,9.53,0,1,0,10.77-8A9.49,9.49,0,0,0,142.24,195.4Z"/><path d="M104.53,190a9.53,9.53,0,1,0,10.76-8A9.47,9.47,0,0,0,104.53,190Z"/><path d="M85.65,187.28a9.54,9.54,0,1,0-10.79,8A9.52,9.52,0,0,0,85.65,187.28Z"/><path d="M174.55,171.31a9.47,9.47,0,1,0-8.09-10.71A9.46,9.46,0,0,0,174.55,171.31Z"/><path d="M136.85,165.91a9.46,9.46,0,1,0-8.1-10.72A9.46,9.46,0,0,0,136.85,165.91Z"/><path d="M91,149.79a9.54,9.54,0,1,0,10.75-8A9.5,9.5,0,0,0,91,149.79Z"/><path d="M53.29,144.36a9.52,9.52,0,1,0,10.77-8A9.52,9.52,0,0,0,53.29,144.36Z"/><path d="M109.85,85.52a9.46,9.46,0,1,0-8.09-10.7A9.51,9.51,0,0,0,109.85,85.52Z"/><path d="M72.12,80.13A9.48,9.48,0,1,0,64,69.38,9.49,9.49,0,0,0,72.12,80.13Z"/><path d="M215,291.88a9.46,9.46,0,1,0-8.09-10.71A9.47,9.47,0,0,0,215,291.88Z"/><path d="M169.19,275.77a9.54,9.54,0,1,0,10.77-8A9.53,9.53,0,0,0,169.19,275.77Z"/><path d="M142.24,262.33a9.47,9.47,0,1,0,8.11,10.72A9.48,9.48,0,0,0,142.24,262.33Z"/><path d="M93.78,264.94a9.52,9.52,0,1,0,10.78-8A9.46,9.46,0,0,0,93.78,264.94Z"/><path d="M201.54,251.7a9.47,9.47,0,1,0-8.1-10.72A9.51,9.51,0,0,0,201.54,251.7Z"/><path d="M155.72,235.57a9.52,9.52,0,1,0,10.77-8A9.5,9.5,0,0,0,155.72,235.57Z"/><path d="M128.77,222.13a9.47,9.47,0,1,0,8.08,10.73A9.51,9.51,0,0,0,128.77,222.13Z"/><path d="M91,216.71a9.48,9.48,0,1,0,8.1,10.74A9.54,9.54,0,0,0,91,216.71Z"/><path d="M196.18,356.15a9.52,9.52,0,1,0,10.78-8A9.5,9.5,0,0,0,196.18,356.15Z"/><path d="M166.54,361.45a9.47,9.47,0,1,0-8.08-10.71A9.51,9.51,0,0,0,166.54,361.45Z"/><path d="M182.69,316a9.54,9.54,0,1,0,10.79-8A9.5,9.5,0,0,0,182.69,316Z"/><path d="M155.75,302.54a9.46,9.46,0,1,0,8.07,10.71A9.46,9.46,0,0,0,155.75,302.54Z"/><path d="M107.24,305.13a9.53,9.53,0,1,0,10.8-8A9.49,9.49,0,0,0,107.24,305.13Z"/><path d="M56.06,259.53a9.52,9.52,0,1,0,10.78-8A9.46,9.46,0,0,0,56.06,259.53Z"/><path d="M228.48,332.1a9.47,9.47,0,1,0-8.06-10.73A9.53,9.53,0,0,0,228.48,332.1Z"/><path d="M255.49,412.47a9.46,9.46,0,1,0-8.08-10.71A9.47,9.47,0,0,0,255.49,412.47Z"/><path d="M209.65,396.36a9.53,9.53,0,1,0,10.78-8A9.52,9.52,0,0,0,209.65,396.36Z"/><path d="M242,372.29a9.47,9.47,0,1,0-8.1-10.72A9.52,9.52,0,0,0,242,372.29Z"/><path d="M134.12,50.72A9.46,9.46,0,1,0,126,40,9.47,9.47,0,0,0,134.12,50.72Z"/><path d="M96.39,45.34A9.47,9.47,0,1,0,88.3,34.62,9.5,9.5,0,0,0,96.39,45.34Z"/><path d="M147.59,90.93a9.47,9.47,0,1,0-8.08-10.72A9.48,9.48,0,0,0,147.59,90.93Z"/><path d="M109.9,85.52a9.46,9.46,0,1,0-8.1-10.72A9.46,9.46,0,0,0,109.9,85.52Z"/><path d="M47.87,114.87a9.47,9.47,0,1,0-8.08-10.72A9.46,9.46,0,0,0,47.87,114.87Z"/><path d="M66.8,184.52a9.53,9.53,0,1,0,10.76-8A9.49,9.49,0,0,0,66.8,184.52Z"/><path d="M47.92,181.84a9.53,9.53,0,1,0-10.79,8A9.52,9.52,0,0,0,47.92,181.84Z"/><path d="M53.27,144.35a9.54,9.54,0,1,0,10.75-8A9.5,9.5,0,0,0,53.27,144.35Z"/><path d="M15.56,138.92a9.52,9.52,0,1,0,10.78-8A9.52,9.52,0,0,0,15.56,138.92Z"/><path d="M91,216.69a9.47,9.47,0,1,0,8.08,10.73A9.51,9.51,0,0,0,91,216.69Z"/><path d="M53.3,211.27A9.48,9.48,0,1,0,61.4,222,9.55,9.55,0,0,0,53.3,211.27Z"/>
                            </svg>
                        </li>
                    </ul>
                </span>
            </div>
            <a class="shot-profile" href="index.php?page=user&username=' . $username . '">
                <img src="assets/user-uploads/user-profiles/' . $profile_picture . '" alt="Profielfoto van ' . $username . '">
                <span class="shot-user">' . $username . '</span>
            </a>
            <script>
                // STRIP UPLOAD (SHOT) TITLE IF MORE THEN 18 CHARACTERS
                var shotTitle = document.querySelector("#shot-info-' . $upload_id . '");
                var shotTitleValue = shotTitle.textContent;
                if (shotTitleValue.length > 18) {
                    var shotTitleAltered = shotTitleValue.substr(0, 18); // https://stackoverflow.com/questions/7708819/keep-only-first-n-characters-in-a-string
                    shotTitleFinal = shotTitleAltered + "..";
                    shotTitle.textContent = shotTitleFinal;
                }

                // SHOT PIN BUTTON TOGGLE
                var shotPinButton = document.querySelector(".shot-' . $upload_id . ' .shot-info .shot-actions li.shot-pin");
                shotPinButton.onclick = function() {
                    if (this.classList.contains("active")) {this.classList.remove("active");
                    } else {this.classList.add("active");}
                };
                // SHOT HEART BUTTON TOGGLE
                var shotHeartButton = document.querySelector(".shot-' . $upload_id . ' .shot-info .shot-actions li.shot-heart");
                shotHeartButton.onclick = function() {
                    if (this.classList.contains("active")) {this.classList.remove("active");
                    } else {this.classList.add("active");}
                };
            </script>
        </div>  
        ';
        

}

    
    
    
    
    echo '
    </div>


    </section>


    
    
    ';

}

?>


<script>
    document.querySelector('body').classList.add('user-profile');
</script>